...
Los CSV de la importación (HU-7) son la excepción a que los datos se generen: son archivos estáticos commiteados en `tests/soporte/csv/`, porque son exactamente la planilla que subiría un comercio y se revisan de un vistazo en el PR. Pueden tener códigos de barras fijos justamente porque cada test trae su propio comercio: el índice único es `(comercio_id, codigo_barras)`, así que dos tests con el mismo archivo nunca se ven entre sí. Lo que se espera de cada archivo —números de fila, motivos de rechazo— vive en `tests/soporte/csv.js` y no en los specs, para que agregarle una fila a un CSV no deje una aserción apuntando a otra cosa.
...
- `tests/e2e/importacion.spec.js` — HU-7 (SCRUM-82): la importación del catálogo inicial recorrida por la pantalla `/productos/importar`, entrando por el link real del catálogo. Cubre el CSV válido completo (con los encabezados escritos de todas las formas en que llega una planilla real —acentos, camelCase, mayúsculas— y una columna `precio` que se ignora sin rechazar el archivo); el archivo con filas mixtas, que es el corazón de la historia: las tres rechazadas se informan con su número de línea y su motivo, y las cuatro válidas entran igual —una de ellas *después* de las tres malas, así que un aborto se vería—; que lo importado queda en el catálogo y cada producto con su fila en `STOCK`, incluido el que trae `stock_actual` vacío, que debe quedar en 0 pero con su fila creada; y el CSV exportado con `;` y BOM, que es el que sale de Excel en es-AR.
- `tests/api/importacion-csv.spec.js` — HU-7, el contrato de `POST /api/productos/importar`. Verifica los 400 —los que invalidan el archivo entero: falta una columna obligatoria, no hay filas de datos— afirmando explícitamente que **no** son un 200 con errores por fila, que es la distinción que pide la historia; el invariante `importados + fallidos === procesadas` en todos los 200; y que volver a subir el mismo archivo no duplica nada, sino que reporta cada fila como código de barras repetido.

Los dos casos de "falta una columna obligatoria" son el mismo criterio partido en dos, y conviene saber por qué: el Frontend lee el archivo antes de mandarlo, así que ese CSV **nunca llega al backend desde la pantalla** — el spec de `e2e` verifica que la pantalla lo frena sin subir nada, y el de `api` que el endpoint igual lo rechaza con 400 para cualquier cliente que no sea esa pantalla.

El caso `interrumpido: true` **queda sin cubrir**, y es a propósito. Ese estado se activa solo si `crearProducto` lanza, a mitad del archivo, algo que no es un `ErrorDeNegocio` de menos de 500 —un bug propio o la base caída— y además ya hay filas commiteadas. Por HTTP no se puede pedir: todo lo que un cliente manda deriva en un error de fila, que por diseño no interrumpe; y la base es Neon, remota y compartida, no un contenedor del compose que se pueda pausar. `Backend/tests/importacion.test.js` lo prueba llamando al service directo, y `Frontend/src/components/ResumenImportacion.test.jsx` prueba cómo se muestra: los dos lugares correctos. Lo que sí queda verificado acá es su consecuencia observable —el invariante se afirma en todos los 200— y la recuperación que documenta el endpoint, volver a subir el mismo archivo, que se prueba de verdad contra el stack.
